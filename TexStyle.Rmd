...
\item Equations in display math mode need punctuations. (Comma and period.)
...
\item Use ``$\backslash$dd'' for differentiation operator.
...
\item When revise the paper, answer how the comments are addressed, in blue. 
...
\item Don't use red and green for different in plots in order to be color blind 
      friendly.
...
\item Use booktabs effectively. Don't use ``hline'' in table. Use ``toprule'', 
      ``midrule'', ``bottomrule''.
